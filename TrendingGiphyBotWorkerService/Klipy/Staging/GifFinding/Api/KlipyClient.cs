...
) : IKlipyClient
{
    ...
    {
        var result = await _httpClient.GetAsync($"trending?page={page}&per_page={perPage}&customer_id={_appConfig.CurrentValue.Klipy.CustomerId}&locale={locale}&format_filter={formatFilter}", cancellationToken);
    ...
    {
        var result = await _httpClient.GetAsync($"search?page={page}&per_page={perPage}&q={query}&customer_id={_appConfig.CurrentValue.Klipy.CustomerId}&locale={locale}&format_filter={formatFilter}", cancellationToken);
        var stream = await result.Content.ReadAsStreamAsync(cancellationToken);
    ...
    {
        var result = await _httpClient.GetAsync($"search?page={page}&per_page={perPage}&customer_id={_appConfig.CurrentValue.Klipy.CustomerId}&locale={locale}&content_filter={contentFilter}&format_filter={formatFilter}", cancellationToken);
        var stream = await result.Content.ReadAsStreamAsync(cancellationToken);
