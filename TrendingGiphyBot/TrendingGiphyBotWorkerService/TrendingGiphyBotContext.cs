...
public class TrendingGiphyBotContext : DbContext, ITrendingGiphyBotContext
{
	...
	{
		return await Database.ExecuteSqlInterpolatedAsync($"DELETE FROM UrlCache WHERE Stamp < {oldestDate}");
	}
	public async Task<int> DeleteUrlHistoriesOlderThan(DateTime oldestDate)
	{
		return await Database.ExecuteSqlInterpolatedAsync($"DELETE FROM UrlHistory WHERE Stamp < {oldestDate}");
	}
	...
	public async Task<List<UrlHistoryContainer>> InsertUrlHistories(List<UrlHistoryContainer> containers)
	{
		...
		if (Database.GetDbConnection() is not SqlConnection sqlConnection)
			//TODO correct this thrown exception
		...
		using var bulkCopy = new SqlBulkCopy(sqlConnection, SqlBulkCopyOptions.Default, sqlTransaction) { BulkCopyTimeout = 600 };
	...
	{
		await Database.ExecuteSqlInterpolatedAsync($"DELETE FROM JobConfig WHERE ChannelId = {channelId}");
	}
